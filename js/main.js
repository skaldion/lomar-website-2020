...
$(document).ready(function () {
  ...
  var i = 0,
    iOS = false,
    android = false,
    iOSTest = ["ipad", "iphone", "ipod"],
    androidTest = ["android", "linux"];

  var ua = navigator.userAgent.toLowerCase();
  var platform = navigator.platform.toLowerCase();

  for (; i < iOSTest.length; i++) {
    //		if ( ua.indexOf(iOSTest[i]) > -1 ) {
    if (platform === iOSTest[i]) {
      iOS = true;
      break;
    }
  }
  i = 0;
  for (; i < androidTest.length; i++) {
    //		if ( ua.indexOf(androidTest[i]) > -1 ) {
    //		if( platform === androidTest[i] ) {
    if (
      platform.indexOf(androidTest[i]) != -1 ||
      ua.indexOf(androidTest[i]) != -1
    ) {
      android = true;
      break;
    }
  ...
  //	$( "#print" ).append( ua + " " + platform  );
  //	$( "#print" ).append( android + " "+ iOS  );

  if (!android && !iOS) {
    ////	if (iOS) {
    $("#intro").parallax("50%", 0.3);
    $("#mision-vision").parallax("50%", 0.2);
    $("#fiscal").parallax("50%", 0.2);
    $("#filosofia").parallax("50%", 0.2);
    $("#otros-servicios").parallax("50%", 0.2);
  }
  ...
  $("a[href*=#]").on("click", function (e) {
    ...
    if (parseInt(scrollTarget) !== parseInt($(window).scrollTop())) {
      $("html,body").animate({ scrollTop: scrollTarget }, 1000, "swing");
    }
  });
});
/*!
